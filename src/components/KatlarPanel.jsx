...
export default function KatlarPanel({ tesisId, tenantId }) {
  ...
  const toggleAlanTipi = (tip) => {
    setForm(f => ({
      ...f,
      alan_tipleri: f.alan_tipleri.includes(tip)
  ...
  const handleDuzenle = (kat) => {
    setForm({
      kat_adi: kat.kat_adi || '',
      sozlesme_m2: kat.sozlesme_m2 || '',
      kullanilabilir_m2: kat.kullanilabilir_m2 || '',
      tavan_yuksekligi: kat.tavan_yuksekligi || '',
      alan_tipleri: kat.alan_tipleri || [],
      asmakat_var: kat.asmakat_var || false,
      asmakat_m2: kat.asmakat_m2 || '',
      asmakat_aciklama: kat.asmakat_aciklama || '',
    })
    setDuzenleId(kat.id)
    setFormAcik(true)
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          onClick={() => { setForm(bos); setDuzenleId(null); setFormAcik(!formAcik) }}
      ...
      ) : katlar.length === 0 && !formAcik ? (
        <div className="text-xs text-gray-400 py-2">Henüz kat tanımlı değil.</div>
      ) : (
        <div className="space-y-2 mb-3">
          {katlar.map(kat => (
            <div key={kat.id} className="bg-white rounded-lg border border-gray-200 px-4 py-3">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium text-gray-700">{kat.kat_adi}</div>
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xs text-gray-400">
                      Sözleşme: <span className="text-gray-600">{kat.sozlesme_m2?.toLocaleString('tr-TR')} m²</span>
                    </span>
                    <span className="text-xs text-gray-400">
                      Kullanılabilir: <span className="text-gray-600">{kat.kullanilabilir_m2?.toLocaleString('tr-TR')} m²</span>
                    </span>
                    {kat.tavan_yuksekligi > 0 && (
                      <span className="text-xs text-gray-400">
                        Tavan: <span className="text-gray-600">{kat.tavan_yuksekligi}m</span>
                      </span>
                    )}
                    {kat.asmakat_var && (
      ...
      {formAcik && (
        <div className="bg-white rounded-lg border border-blue-200 p-4 space-y-3">
          <div className="text-xs font-medium text-blue-700 mb-2">
            {duzenleId ? 'Katı Düzenle' : 'Yeni Kat'}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <input
                value={form.kat_adi}
                onChange={e => set('kat_adi', e.target.value)}
            ...
            <div>
              ...
              <input
                type="number"
                value={form.sozlesme_m2}
                onChange={e => set('sozlesme_m2', e.target.value)}
                className="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-400"
                placeholder="5000"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="number"
                value={form.kullanilabilir_m2}
                onChange={e => set('kullanilabilir_m2', e.target.value)}
                className="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-400"
                placeholder="4800"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="number"
                step="0.1"
                value={form.tavan_yuksekligi}
                onChange={e => set('tavan_yuksekligi', e.target.value)}
                className="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-400"
                placeholder="8.5"
              />
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {ALAN_TIPLERI.map(tip => (
                <button
                  key={tip}
                  onClick={() => toggleAlanTipi(tip)}
                  className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                    form.alan_tipleri.includes(tip)
          ...
          <div>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={form.asmakat_var}
                onChange={e => set('asmakat_var', e.target.checked)}
                className="rounded"
              />
            ...
            {form.asmakat_var && (
              <div className="grid grid-cols-2 gap-3 mt-2">
                <div>
                  ...
                  <input
                    type="number"
                    value={form.asmakat_m2}
                    onChange={e => set('asmakat_m2', e.target.value)}
                    className="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-400"
                    placeholder="0"
                  />
                </div>
                <div>
                  ...
                  <input
                    value={form.asmakat_aciklama}
                    onChange={e => set('asmakat_aciklama', e.target.value)}
                    className="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-400"
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button
              onClick={() => { setFormAcik(false); setForm(bos); setDuzenleId(null) }}
              className="text-xs px-3 py-1.5 text-gray-500 hover:bg-gray-50 rounded-lg"
            >
              İptal
            </button>
            <button
              onClick={handleKaydet}
              className="text-xs px-4 py-1.5 bg-blue-700 text-white rounded-lg hover:bg-blue-800"
            >
