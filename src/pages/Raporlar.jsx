...
export default function Raporlar() {
  ...
  const tipOzet = ['depolama', 'transport', 'vas', 'karma'].map(tip => {
    const tipSoz = sozlesmeler.filter(s => s.tip === tip && s.durum === 'aktif')
    const m2Toplam = tipSoz.reduce((acc, s) => acc + (Number(s.dep_m2_birim_fiyat) || 0), 0)
    return { tip, sayi: tipSoz.length, label: TIP_LABEL[tip] }
  }).filter(t => t.sayi > 0)
  ...
  ihaleler.forEach(i => {
    if (i.mevcut_3pl) {
      rakip3pl[i.mevcut_3pl] = (rakip3pl[i.mevcut_3pl] || 0) + 1
    }
  })
  ...
  const handleExport = () => {
    const ciData = musteriCiro.map(m => ({
      'Müşteri': m.musteriAd,
      'Toplam Sözleşme': m.toplamSozlesme,
      'Aktif Sözleşme': m.aktifSozlesme,
      'Risk': m.risk,
    }))
      ...
      {aktifTab === 'ihale' && (
        <div className="space-y-6">
          ...
          {rakipListesi.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100">
                <div className="text-xs font-medium text-gray-400 uppercase tracking-wide">Mevcut 3PL Analizi (İhalelerden)</div>
              </div>
              <Tablo
