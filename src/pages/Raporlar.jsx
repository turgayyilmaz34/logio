...
export default function Raporlar() {
  ...
  const analizVerisi = tesisler.map(t => {
    ...
    const musteriDolulukPuani = ilgiliSozlesmeler.reduce((sum, s) => {
      ...
      const sure = Number(s.sure_ay || 12)
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="flex flex-col items-center gap-3">
        ...
        <span className="text-sm font-bold text-gray-400 uppercase tracking-widest">Analiz Motoru Çalışıyor...</span>
      </div>
    </div>
  )
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 bg-white p-10 rounded-[40px] shadow-xl shadow-slate-200/50 border border-white">
          <div className="flex justify-between items-center mb-10">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-black text-slate-800 uppercase tracking-tight text-lg">Tesis Verimlilik Endeksi</h3>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm">
              ...
              <div className="text-3xl font-black text-red-500 mt-1">
                {analizVerisi.filter(r => r.ratio > hedefOran).length}
              </div>
            </div>
            <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm">
              <span className="text-[10px] font-black text-slate-400 uppercase">Boş Kapasite</span>
              <div className="text-3xl font-black text-green-500 mt-1">
                {analizVerisi.filter(r => r.ratio < 1).length}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden mb-20">
          <div className="px-10 py-8 border-b border-slate-50 flex justify-between items-center">
            ...
            <div className="text-xs font-bold text-slate-400 bg-slate-50 px-4 py-2 rounded-full">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/50">
              <tr>
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">İşletme Modeli</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Mülkiyet</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">White Space (Ratio)</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Eylem</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {analizVerisi.map((item, idx) => (
                <tr key={idx} className="hover:bg-slate-50/80 transition-all group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-2xl ${item.yapi === 'Çelik' ? 'bg-amber-50 text-amber-600' : 'bg-slate-100 text-slate-600'}`}>
                        <Box size={20} />
                      </div>
                      <div>
                        <div className="font-black text-slate-700 text-sm">{item.name}</div>
                        <div className="text-[10px] font-bold text-slate-400 uppercase">{item.yapi} Konstrüksiyon • {item.alan.toLocaleString()} m²</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-slate-600 uppercase tracking-tighter">{item.isletme}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2">
                      <Home size={14} className="text-slate-300" />
                      <span className="text-xs font-bold text-slate-600 uppercase tracking-tighter">{item.mulkiyet}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6 text-center">
                    <div className="flex flex-col items-center">
                      <span className={`text-xl font-black ${item.ratio > hedefOran ? 'text-red-500' : 'text-blue-600'}`}>
                        {item.ratio}
                      </span>
                      <div className="w-16 h-1 bg-slate-100 rounded-full mt-1 overflow-hidden">
                        <div 
                          className={`h-full ${item.ratio > hedefOran ? 'bg-red-500' : 'bg-blue-600'}`} 
                          style={{width: `${Math.min(100, (item.ratio / 2) * 100)}%`}}
                        />
                      </div>
                    </div>
                  </td>
